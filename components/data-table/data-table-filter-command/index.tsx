...
export function DataTableFilterCommand<TData, TSchema extends z.AnyZodObject>({
  ...
  return (
    <div>
      ...
      >
        {isLoading ? (
          <LoaderCircle className="mr-2 size-4 shrink-0 animate-spin text-muted-foreground opacity-50 group-hover:text-popover-foreground" />
        ) : (
          <Search className="mr-2 size-4 shrink-0 text-muted-foreground opacity-50 group-hover:text-popover-foreground" />
        )}
